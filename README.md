...
- `personal/.config/wluma/config.toml` - wluma auto-brightness for the Framework ALS and docked DDC monitors. There is no pacman, Flatpak, or AUR `-bin` package; install extra `iio-sensor-proxy`, place the official GitHub `wluma-x86_64-linux-gnu` binary at `~/.local/bin/wluma`, install `root/etc/udev/rules.d/90-wluma-backlight.rules`, and enable `wluma.service`. The config only disables gamma so Omarchy nightlight keeps hyprsunset. wluma learns from Omarchy brightness keys and hyprmoncfg sliders; it does not start adjusting until those have been used a few times in different lighting.
